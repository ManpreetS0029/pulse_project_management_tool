...
export default function CreateProjectModal({
  ...
  useEffect(() => {
    if (isOpen) {
      if (existingProject) {
        let formattedDate = getDefaultDueDate();
        if (existingProject.dueDate) {
          try {
            formattedDate = new Date(existingProject.dueDate)
              .toISOString()
              .split('T')[0];
          } catch (e) {
            formattedDate = existingProject.dueDate;
          }
        }
        reset({
          name: existingProject.name || '',
          workspace:
            existingProject.workspaceName ||
            (typeof existingProject.workspace === 'string'
              ? existingProject.workspace
              : '') ||
            defaultWorkspace,
          description: existingProject.description || '',
          status: existingProject.status || 'In Progress',
  ...
  }, [isOpen, defaultWorkspace, existingProject, reset]);
  ...
  const onSubmit = async (data) => {
    ...
    if (isEditing) {
      const projId = existingProject._id || existingProject.id;
      ...
      try {
        const res = await apiPrivate.put(`/project/${projId}`, updatedPayload);
        const returnedData = res.data?.data || res.data;
        const savedProject = {
          ...existingProject,
          ...updatedPayload,
          id: returnedData._id || returnedData.id || projId,
          _id: returnedData._id || returnedData.id || projId,
          name: returnedData.name || updatedPayload.name,
          workspace: returnedData.workspaceName || data.workspace,
          workspaceName: returnedData.workspaceName || data.workspace,
          description:
            returnedData.description !== undefined
              ? returnedData.description
              : updatedPayload.description,
          status: returnedData.status || updatedPayload.status,
          priority: returnedData.priority || updatedPayload.priority,
          progress: returnedData.progress ?? existingProject.progress ?? 0,
          dueDate: returnedData.dueDate || updatedPayload.dueDate,
          tasksTotal:
            returnedData.tasksTotal ?? existingProject.tasksTotal ?? 0,
          tasksCompleted:
            returnedData.tasksCompleted ?? existingProject.tasksCompleted ?? 0,
        };
        onSave(savedProject, true);
        onClose();
      } catch (error) {
      ...
      try {
        ...
        onSave(createdProject, false);
        onClose();
      } catch (error) {
        console.error('Error creating project:', error);
        onSave(newProject, false);
        onClose();
      } finally {
        setIsSubmitting(false);
      }
        ...
        >
          ...
          <div className="px-6 py-5 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
            <div className="flex items-center gap-3">
              <div
                className={`h-10 w-10 rounded-2xl flex items-center justify-center shadow-sm border ${
                  isEditing
            ...
            <div className="pt-4 border-t border-slate-100 flex items-center justify-between gap-3">
              {isEditing && onDelete ? (
                <button
                  ...
                  onClick={() => {
                    onClose();
                    onDelete(existingProject);
                  }}
                  className="inline-flex items-center gap-1.5 px-3.5 py-2.5 rounded-xl text-xs font-bold text-rose-600 hover:text-rose-700 hover:bg-rose-50 border border-rose-200/60 transition cursor-pointer"
                >
                  <svg
              ...
              <div className="flex items-center gap-2.5">
                ...
                <button
                  ...
                  className="px-5 py-2.5 rounded-xl text-xs font-bold text-white shadow-md hover:shadow-lg transition cursor-pointer hover:-translate-y-px disabled:opacity-50 disabled:cursor-not-allowed"
